...
* **100% HTML5 & CSS3 puro**: Sin dependencias externas, frameworks ni JavaScript.
* **Semántica web estricta**: Estructura limpia mediante `<header>`, `<main>`, `<section>`, `<form>`, `<label>`, `<button>`, `<ul>` y `<footer>`.
* **Diseño responsive avanzado**: Adaptación fluida mediante Flexbox y Media Queries para dispositivos móviles en vertical (portrait), horizontal (landscape) y pantallas de escritorio.
...
* **Temática teatral integrada**: Paleta de colores oscuros (`#0c0812`) y dorados (`#d4af37`), tipografías combinadas (*Cinzel* y *Outfit*) y componentes visuales como la simulación del aforo de la sala y el selector de espectáculo.
* **Accesibilidad (a11y)**: Clases para lectores de pantalla (`.sr-only`), vinculación correcta de etiquetas `<label>` e interactividad visual cuidada.
